...
-update문에 조건을 생략하거나 조건절이 기본키에 대한 조건이 아니면
 mysql 워크밴치에서는 안정성을 위해 update문을 실행하지 않음. 
 실행하려면 워크벤치 기본 설정을 변경(Edit > Preference > SQL Editor)
단, 워크밴치가 아닌 mysql 8.0* command Like Client로 접속해서 해당 쿼리를 실행하면 설정에 상관없이 실행됨

delete from 테이블명 [where 조건];
- 해당 조건을 만족하는 튜플을 삭제
- 조건이 생략되면 전체 튜플을 삭제
- delete문은 사용을 조심해야한다 => select문을 이용해서 삭제된 것처럼 처리할 수 있음*/
