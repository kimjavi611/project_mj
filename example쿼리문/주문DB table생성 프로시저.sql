...
delimiter 기호
create trigger 트리거명 
{before | after}{insert | update | delete} on 트리거명
for each row
begin
	코드작성;
end 기호
...
- 코드 작성 부분에서 ;이 나왔을 때 실행되지 않도록 ;을 무시하게 하기 위해서 사용
*/
...
show triggers;
use example;
/*주문테이블에 주문내역이 추가되면 제품 테아블의 재고량이 변하는 트리거를 작성하세요*/
...
begin
    ...
    */
    /*
    -변수선언 (begin바로 밑에 변수들을 모아서 선언해야함)
...
begin 
    ...
    -- set _order_num = 'o13';
    insert into `주문` values(_order_num, id, _product, _amount, _address, _date);
	select * from `주문`;
end //
delimiter ;
call addOder('apple','p01',15,'충북 청주시', '2021-06-01');
