...
-- select avg(수량) as 'apple의 평균주문량' from 주문 where 주문고객 = 'apple';
-- group by는 where절 다음에 order by 앞에 있어야 한다
-- 주문 제품별 총 판매수량
-- select 주문제품, sum(수량) as 주문수량 from 주문 group by 주문제품;
-- 고객별 총 주무수량
-- select 주문고객, sum(수량) as '판매수량(누적)' from 주문 group by 주문고객;
-- 제조업체별 제품수와 제품들의 최고가 최저가를 확인
