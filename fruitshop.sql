...
CREATE TABLE if not exists `trade` (
	...
	`tr_date`	datetime not NULL default current_timestamp, -- database에 등록된 시간의 값이 자동으로 들어감 타입이 datetype일때만 사용 가능
...
select * from fruit;
-- (A과수원에서) 사과 100상자 입고되었다. 상자당 가격은 10,000원
-- trade 테이블애 insert후, fruit 테이블에 재고량을 update
...
select * from fruit;
-- 손님이 사과 1상자를 상자당 20,000원에 구매를 했다.
-- trade 테이블에 insert, fruit 테이블에 update
/*insert into trade(tr_num,tr_amount, tr_type, tr_price,tr_fr_name) 
...
select * from fruit;
-- 오늘 과일 판매액을 확인
select sum(tr_price) as '판매액' from trade where tr_type = 'sell'
and tr_date like '2021-06-02%';
-- 총 매출액
/*case when 사용법
...
select * from fruit;
/*입고될 때 없는 과일이 입고 되는 경우 fruit 테이블에 새로운 과일을 추가*/
...
begin
	declare _count int default 0; -- 구매하려는 과일이 있는지 앖는지 확인
    declare _num int;
