...
insert into salary
	values('사원',200,10),('대리',220,15),('과장',240,20),('부장',280,25),('전무',300,30),
    ('이사',350,35);
...
-- 해당 사원의 월급을 변경
    ...
    declare _add int default 0; -- add : 호봉 추가금
    declare _salary int default 0; -- 월급
    -- 기본급 가져오기
    set _base = (select sa_base_salary from salary where sa_level = _level);
    -- 호봉 기본급 가져오기
    set _add = (select sa_base_salary from salary where sa_level = _level);
    -- 연차에 맞게 호봉 추가금 계산하기
    set _add = _add * _year;
    -- 사원의 직급과 년차, 월급을 수정
    update employee set
