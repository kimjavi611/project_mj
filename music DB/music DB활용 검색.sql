...
singer 테이블(가수이름), source 테이블(노래명), sing 테이블(가수번호, 음원번호)
select 속성 from 테이블;
...
member테이블 me_num, 외래키 me_si_mem 
singer 테이블(소녀시대),  artist 테이블(멤버이름) member 테이블 */
...
소녀시대의 노래들 중 3집 앨범에있는 노래들
album table과 list 테이블 확인*/
 select so_title from singer
	join sing
		on si_num = sn_si_num
	join `source`
...
/*노래제목 또는 가사에 거짓말이 포함된 노래를 확인하는 쿼리문 */
select so_title from `source` where so_title like '%거짓말%'
or so_lyric like '%거짓말%';
...
join이 가능한 경우 순서가 바뀌어도 상관없음*/
select si_name from `member`
	join artist
		on ar_num = me_ar_num
	join singer
		on si_num = me_si_num
	where ar_num like '%태연%';
    
    /*태연이 피처링한 노래들을 확인하는 쿼리문
    singer 테이블 (태연),sing 테이블(피처링한 노래) source 테이블 (노래)*/
    select * from singer
...
singer(소녀시대), source(타이틀) sing(노래) */
select so_title from singer
	join sing
		on si_num = sn_si_num
	join `source`
