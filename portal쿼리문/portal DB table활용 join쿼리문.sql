...
-- 2020135001 학생이 수강한 강의 코드를 하나의 테이블로 확인할 수 있다(o),
-- =>수강 테이블에서 확인
-- select 테이블1.속성1, 테이블1.속성2,..테이블2.속성1 from 테이블1, 테이블2 where 조건
-- 2020135001 학생이 수강한 강의 제목을 확인하기 위해서 course(수강정보), class(강의제목)
-- 테이블이 필요 
...
-- => 강의명과 수강정보가 있어야함
select st_name from course, class, student where co_cl_code = cl_code and co_st_num = st_num
and cl_pr_num = 'p2000135001';
-- select 속성1,속성2,... from 테이블1 join 테이블2 on 테이블1.외래키 = 테이블2. 기본키;
-- 위에서 한 2020135001 학번 학생이 수강한 강의명을 확인하는 코드를 join을 이용하여 확인
select course.co_st_num as '학번', class.cl_title as '강의명' 
from course join class on course.co_cl_code = class.cl_code
where course.co_st_num = '2020135001';
-- join 할 때 두 테이블의 속성명이 겹치지 않는 경우 속성 이름 앞에 테이블명을 생략해도 된다
