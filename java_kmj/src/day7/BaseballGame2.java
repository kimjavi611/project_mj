...
public class BaseballGame2 {
	...
	public static void main(String[] args) {
		...
		}while(strike < 3);// strike != 3 => 4,5가 나올수도있다
		scan.close();
	}
	 ...
	 * 메소드명 : contains
	 * 설명    : [1,2,3,4,5] 배열에서 처음부터 3개중에서 7이 있는지 없는지 확인 => 없음
	...
	/* 기능 : 배열 arr가 주어지면 주어진 배열에 min~max사이의 랜덤한 수를 생성하여 "중복되지 않게"(배열에 저장전에 확인) 저장하는 메소드
	 * 매개변수 : 배열 arr, 랜덤범위(mix~max) => int[]arr. int min, int max
	 * 리턴타입 : 없음 => void
	 * 메소드명 : init*/
	public static void init(int[]arr,int min, int max) {
		int cnt = 0;//배열에 저장된 숫자의 갯수
		while(cnt<arr.length) {//같을때까지 => 같으면 멈춘다
			int r =(int)(Math.random()*(max-min+1)+ min);//랜덤한수 생성
			//중복되지 않는지 확인
	...
	/* 기능 	  : 배열 arr1과 arr2에서 같은 위치의 숫자를 비교하여 같은 숫자의 갯수를 알려주는 메소드 
	 * 매개변수 : 배열 arr1,배열 arr2 => int[]arr1, int []arr2
	 * 리턴타입 : 같은 숫자의 갯수 => 정수 => int
	 * 메소드명 : getStrike
	 * 설명 	  : [1 2 3][3 4 5] => 0
	 * 			[1 2 3][1 9 4] => 1 두 배열을 비교*/
	 ...
	 * 메소드명 : getBall
	 * 설명 	  : [1 2 3][3 2 4] => 1 => 같은 숫자의 개수 - 스트라이크의 수
	 * 									2 - 1 = 1 */
