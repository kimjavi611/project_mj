...
public class ListTest1 {
	...
	public static void main(String[] args) {
		...
		//제네릭 클래스는 타입을 클래스로 해야 한다. 일반 변수 x=> Wrapper클래스 이용 
		ArrayList<String> list = new ArrayList<String>();//<클래스명> 생략가능 , 써주는게 좋음.
		//값지정 안했을때 기본 10개짜리 배열 => 마우스올려서 확인할 수 있음
		//데이터가 꽉차면 알아서 배열 크기를 늘려줌
		//daa(): 리스트에 추가 
		for(int i = 0 ; i<10 ; i++) {
			list.add("학원 출근");
		}
		...
		System.out.println("리스트 크기 : " + list.size());//=>지금까지 몇개 들어가있는지 크기를 알려줌
		list.add(10,"아침");
		System.out.println(list.get(10));
		...
		list.set(10,"간식");
		System.out.println(list.get(10));
		System.out.println("리스트 크기 : " + list.size());//덮어쓰기 때문에 크기가 변하지 않음
		//indexOf(값) : 값이 리스트에 있는지 없는지 확인하여 있으면 번지를 없으면 -1을 리턴. 
		//해당 클래스의 equals()를 호출하여 같은지를 확인
